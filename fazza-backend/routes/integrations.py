...
@router.get("/companies/by-instagram-account/{account_id}")
async def get_company_by_instagram(account_id: str, db: AsyncSession = Depends(get_db)):
    # In a real implementation, this would join Company with InstagramAccount
    # Returning mock data as requested for the workflow logic
    return {
        "id": "company_456",
        "name": "Fazza CRM Demo",
        "instagram_page_id": account_id,
        "instagram_access_token": "EAAZA...", # Should come from DB
        "settings": {
            "custom_field_1": "Valor Premium",
