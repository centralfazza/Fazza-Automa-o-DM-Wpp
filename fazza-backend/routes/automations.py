...
async def match_automation(
    ...
    for auto in automations:
        ...
        # Specific trigger type match (e.g., new_follower, comment)
        if trigger_type and trigger.get('type') == trigger_type:
            # If it's a keyword trigger, still check message content
        ...
        # Legacy fallback if trigger_type not sent or generic keyword match
        if not trigger_type:
            if trigger.get('type') == 'keyword' and message:
                keywords = trigger.get('keywords', [])
                if any(kw.lower() in message.lower() for kw in keywords):
                    return auto
