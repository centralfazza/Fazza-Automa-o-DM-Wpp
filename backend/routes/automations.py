...
async def match_automation(
    ...
    for auto in automations:
        ...
        # Generic Trigger Type Match (e.g. comment, story_mention)
            ...
            # If it's a comment, we might still want to check keywords if provided
            if trigger_type == 'comment' and message:
                keywords = trigger.get('keywords', [])
                if isinstance(keywords, list) and keywords:
                    ...
                    for kw in keywords:
                        ...
                        if kw_clean.startswith('!'):
                            ...
                            matched = True
                            break
                    if not matched: continue # Skip this automation if keyword doesn't match
            return auto
